...
| `fast` | critical | 快速验证子集（当前配置 40 个模块、911 个用例） |

**测试统计（当前配置）**: 33组（含 `fast`）。`--all` 默认排除重复的 `fast` 组；最近一次（2026-08-07）使用 Anaconda `python39` 执行 `--all-with-fast` 实测为 33组、123个模块、2361个用例，100% 通过，耗时 1013.0 秒；具体以本地运行报告为准。
