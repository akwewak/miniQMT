...
### 清仓后的自动暂停  [v3.8.9 / v3.9.0]

动态止盈止损触发**清仓类卖出**并收到真实成交确认后，系统默认会自动暂停同一股票的活跃网格会话，防止清仓后网格因为价格反弹继续生成卖出信号或提交新网格单。

v3.9.0 起，清仓类信号包括 `take_profit_full`（全仓止盈）与 `stop_loss`（止损，含 `stop_loss_0` 固定止损与 `stop_loss_1` 首次止盈后回落止损）——两者都卖出 `position['available']` 全量，属同一清仓语义。`take_profit_half`（首次止盈只卖 60%）仍有持仓，**不**触发暂停。

| 项 | 行为 |
|----|------|
| 配置开关 | `ENABLE_PAUSE_GRID_AFTER_TAKE_PROFIT_FULL`，默认 `True`（止损联动复用同一开关，未新增配置项） |
| 触发时机 | `take_profit_full` / `stop_loss` 委托成交确认后；不是信号生成时，也不是委托提交时 |
