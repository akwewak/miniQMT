...
### 清仓（全仓止盈 / 止损）与网格联动  [v3.8.9 / v3.9.0]

动态止盈进入 `take_profit_full`、或触发 `stop_loss` 止损清仓后，如果该卖出委托收到本机跟踪订单的成交确认，系统默认暂停同一股票的活跃网格会话（v3.9.0 起两者行为一致，因为都卖出 `available` 全量）：

```python
ENABLE_PAUSE_GRID_AFTER_TAKE_PROFIT_FULL = True
```

该联动只在成交确认链路中执行，避免“信号已生成但委托未成交”时提前暂停网格。`take_profit_half`（首次止盈卖 60%）仍持仓，不触发暂停；网格自身的买卖成交也不会暂停自己。暂停动作等价于 `grid_trading_sessions.enabled=False`：会话仍保持 active，账本、统计和参数都保留，后续可在 Web 网格配置对话框中恢复“自动”。如果没有活跃网格会话，或会话已经暂停，系统会幂等跳过。

如需关闭该联动，可将同名环境变量设为 `false`，或调用 `/api/config/save` 保存：

```json
{"pauseGridAfterTakeProfitFull": false}
```
