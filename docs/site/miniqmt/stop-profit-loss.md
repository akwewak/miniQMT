...
动态止盈进入 `take_profit_full` 后，如果该卖出委托收到本机跟踪订单的成交确认，系统默认暂停同一股票的活跃网格会话：

```python
ENABLE_PAUSE_GRID_AFTER_TAKE_PROFIT_FULL = True
```

该联动只在成交确认链路中执行，避免“信号已生成但委托未成交”时提前暂停网格。暂停动作等价于 `grid_trading_sessions.enabled=False`：会话仍保持 active，账本、统计和参数都保留，后续可在 Web 网格配置对话框中恢复“自动”。如果没有活跃网格会话，或会话已经暂停，系统会幂等跳过。
