...
项目测试代码位于 [test/](https://github.com/weihong-su/miniQMT/tree/main/test) 目录，使用标准 `unittest`。当前回归配置见 `test/integration_test_config.json`，包含 33 个测试组（含 `fast` 快速子集）。

最近一次（2026-08-07）使用 `--all-with-fast` 实测：**33 组、123 模块、2361 用例、2361 通过、0 失败、0 错误、0 跳过，成功率 100%**，耗时 1013.0 秒。
...
| `fast` | critical | 5 分钟快速验证子集（当前 40 个模块、911 个用例） |
