...
项目集成了完整的回归测试框架,支持按模块运行、快速验证和失败重试。当前配置包含 31 个测试组（含 `fast` 快速子集）；`--all` 默认排除重复的 `fast` 组，`--all-with-fast` 会连同快速子集一起运行。最近一次（2026-08-01）使用 Anaconda `python39` 执行 `--all-with-fast` 实测为 31 组、108 个模块、2014 个用例，100% 通过。
