...
# 12. 实盘调试窄接口清理
# =====================================================================
class TestDebugLiveSellApisRemoved(WebAPITestBase):
    """确认实盘调试窄接口不再注册，避免暴露真实账号或触发实盘交易。"""

    def test_debug_live_sell_routes_are_not_registered(self):
        rules = {rule.rule for rule in web_server.app.url_map.iter_rules()}
        self.assertNotIn('/api/debug/live-sell-100-guard', rules)
        self.assertNotIn('/api/debug/order-probe-000799', rules)
        self.assertFalse(hasattr(web_server, 'debug_live_sell_100_guard'))
        self.assertFalse(hasattr(web_server, 'debug_order_probe_000799'))
